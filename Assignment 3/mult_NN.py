...
def buildGraph():
    ...
    with tf.variable_scope("hidden_layer1"):
        hidden_layer1 = tf.nn.relu(build_layer(X_flatten, num_hidden_units))
    ...
    W1 = tf.get_default_graph().get_tensor_by_name("hidden_layer1/Weights:0")
    W2 = tf.get_default_graph().get_tensor_by_name("hidden_layer2/Weights:0")
    W3 = tf.get_default_graph().get_tensor_by_name("softmax_layer/Weights:0")
    regularizer = (tf.reduce_sum(W1*W1) * weight_decay * 0.5) + (tf.reduce_sum(W2*W2) * weight_decay * 0.5) + (tf.reduce_sum(W3*W3) * weight_decay * 0.5)
...
# File setup
f = open("2H_stats.txt", "w+")

# Train
X, Y, ce_loss, total_loss, error, train = buildGraph()
sess = tf.Session()
...
print("END OF RUN")
print("Results - final valid error: {}".format(valid_loss[-1]))
...
plt.legend(loc="best")
fig_loss.savefig("1_2_2_loss.png")
plt.show()
...
plt.legend(loc="best")
fig_error.savefig("1_2_2_error.png")
plt.show()

fig_test_loss = plt.figure(3)
plt.title = 'Test Loss vs. Epoch'
plt.ylabel('Cross Entropy Loss')
plt.xlabel('Epoch')
plt.grid(True)

test_loss_h1 = np.load("H_1000_test_loss.npy")
plt.plot(x_axis, test_loss_h1, '-', label=('1 Hidden Layer'))
plt.plot(x_axis, test_loss, '-', label=('2 Hidden Layers'))

plt.legend(loc="best")
fig_test_loss.savefig("1_2_2_test_loss_cmp.png")
plt.show()

fig_test_loss = plt.figure(4)
plt.title = 'Test Error vs. Epoch'
plt.ylabel('Classification Error')
plt.xlabel('Epoch')
plt.grid(True)

test_err_h1 = np.load("H_1000_test_err.npy")
plt.plot(x_axis, test_err_h1, '-', label=('1 Hidden Layer'))
plt.plot(x_axis, test_err, '-', label=('2 Hidden Layers'))

plt.legend(loc="best")
fig_test_loss.savefig("1_2_2_test_err_cmp.png")
plt.show()
