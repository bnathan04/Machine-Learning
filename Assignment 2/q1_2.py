...
# Train
for count, size in enumerate(batch_size):

    # Start tf session, init variables and SGD optimizer function
    sess = tf.Session()
    sess.run(tf.global_variables_initializer())
    optimizer = tf.train.GradientDescentOptimizer(learning_rate=learning_rate).minimize(mse)
    num_batches = num_data // size
    num_epoch = num_train_steps / num_batches
    plot_steps = np.linspace(0, num_epoch, num=num_train_steps)

    # Run training and find start time
    start_time = time.time()
    for step in range(num_train_steps):
        cur_batch_idx = (step % num_batches) * size
        cur_data = trainData[cur_batch_idx:cur_batch_idx + size]
        cur_target = trainTarget[cur_batch_idx:cur_batch_idx + size]
    ...
    # Plot current run
    plot_name = "Batch Size: " + str(size)
    loss_plot = plt.plot(plot_steps, loss_train, plot_lines[count], label=plot_name)
    final_mse = loss_train[num_train_steps - 1]
    print(plot_name, "- final MSE =", final_mse, "Time elapsed:", elapsed_time)

# Save graph
plt.legend(loc="best")
fig.savefig("1_2_loss_batch_size.png")
# plt.show()
